...
var EmployeeComponent = /** @class */ (function () {
    ...
    EmployeeComponent.prototype.editConfirm = function (data) {
        ...
        if (name && name.length > 1 && surname && surname.length > 1) {
            var user = { name: name, surname: surname, active: this.activee };
            if (this.departmente) {
                user['department'] = this.departmente;
            }
            else {
                user['department'] = this.item.department;
            }
            if (this.positione) {
                user['position'] = this.positione;
            }
            else {
                user['position'] = this.item.position;
            }
            if (Object.keys(this.skills).length > 0) {
                user['skills'] = Object.keys(this.skills);
            }
            else {
                user['skills'] = this.item.skills;
            }
            this.store.dispatch(new _store_actions__WEBPACK_IMPORTED_MODULE_3__["editEmployee"]({ data: user, id: data._id }));
        }
    };
